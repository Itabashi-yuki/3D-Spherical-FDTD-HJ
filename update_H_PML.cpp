...
void update_Hth_tilde(double ***Hth_tilde, double ****Hth, double *CHTH_TILDE, double ****check, int n){
    ...
    for(int i = 0; i < Nr; i++){
        for(int j = 1; j <= Nth - 1; j++){
            for(int k = 0; k < Nph; k++){
                Hth_tilde[i][j][k] = Hth_tilde[i][j][k] + r(i + 0.5) * ( Hth[NEW][i][j][k] - Hth[OLD][i][j][k] ) 
                + CHTH_TILDE[i] * ( Hth[NEW][i][j][k] + Hth[OLD][i][j][k] );
                // check[NEW][i][j][k] += 1.0;
            }
        }
    }
    ...
    for(int i = 0; i < Nr; i++){
        for(int j = 0; j < Nth; j++){
            for(int k = 1; k <= Nph - 1; k++){
                Hph_tilde[i][j][k] = Hph_tilde[i][j][k] + r(i+0.5) * ( Hph[NEW][i][j][k] - Hph[OLD][i][j][k] ) 
                + CHPH_TILDE[i] * ( Hph[NEW][i][j][k] + Hph[OLD][i][j][k] );
                // check[NEW][i][j][k] += 1.0;
            }
        }
    }
